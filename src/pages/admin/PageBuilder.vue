        ...
        <div class="bg-card border-b border-border px-4 py-3">
          <div class="flex items-center justify-between">
            <div class="flex items-center space-x-4">
              <h1 class="text-lg font-semibold text-foreground">Page Builder</h1>
              <div class="flex items-center space-x-2">
                <select
                  v-model="selectedPage"
                  @change="loadPage"
                  class="block w-40 pl-3 pr-10 py-2 text-base border-border focus:outline-none focus:ring-primary focus:border-primary sm:text-sm rounded-md bg-secondary text-foreground"
                >
            ...
            <div class="flex items-center space-x-3">
              <div v-if="isLoading" class="flex items-center text-sm text-muted-foreground">
                <svg class="animate-spin -ml-1 mr-2 h-4 w-4 text-muted-foreground" fill="none" viewBox="0 0 24 24">
              ...
              <button
                @click="handlePreview"
                class="inline-flex items-center px-3 py-2 border border-border shadow-sm text-sm leading-4 font-medium rounded-md text-foreground bg-secondary hover:bg-accent focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary"
              >
              ...
              <button
                @click="handleSave"
                :disabled="isLoading"
                class="inline-flex items-center px-3 py-2 border border-transparent text-sm leading-4 font-medium rounded-md text-primary-foreground bg-primary hover:bg-primary/80 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary disabled:opacity-50"
              >
                <svg class="h-4 w-4 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              ...
              <button
                @click="handlePublish"
                :disabled="isLoading"
                class="inline-flex items-center px-3 py-2 border border-transparent text-sm leading-4 font-medium rounded-md text-primary-foreground bg-emerald-600 hover:bg-emerald-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-emerald-500 disabled:opacity-50"
              >
                <svg class="h-4 w-4 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
...
const fetchPageLayout = async (pageName: string) => {
  ...
  try {
    ...
    showToast({
      variant: 'destructive',
      title: 'Error loading page layout',
...
const handleSave = async () => {
  ...
  try {
    ...
    console.log('Session in handleSave:', session);
    if (!session) {
      showToast({
        variant: 'destructive',
...
const savePageLayout = async () => {
  ...
  try {
    const { data: { session } } = await supabase.auth.getSession();
    console.log('Session in savePageLayout:', session);
    if (!session) {
      showToast({
        variant: 'destructive',
        title: 'Authentication Error',
        description: 'You must be logged in to save page layout.'
      });
      saveStatus.value = 'Error';
      return;
    }
    ...
    if (!res.ok) {
      const errorData = await res.json();
      throw new Error(errorData.message || 'Failed to save page layout');
    }
    saveStatus.value = 'Saved';
  } catch (err: any) {
    showToast({
      variant: 'destructive',
      title: 'Error saving page layout',
      description: err.message
    });
    saveStatus.value = 'Error';
  }
