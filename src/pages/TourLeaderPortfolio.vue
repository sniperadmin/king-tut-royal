<template>
  <div class="container mx-auto p-4">
    ...
    <div v-else-if="leader" class="bg-card rounded-lg shadow-lg p-6 md:p-8 lg:p-10">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            ...
            <ul class="list-disc list-inside text-foreground space-y-2">
              <li>{{ leader.experience || 'Years of experience in luxury travel guidance.' }}</li>
            </ul>
          </div>
          <div>
            <h3 class="text-xl font-semibold text-primary-foreground mb-3">Certifications</h3>
            <ul class="list-disc list-inside text-foreground space-y-2">
              <li v-for="(cert, index) in leader.certifications" :key="index">{{ cert }}</li>
              <li v-if="!leader.certifications || leader.certifications.length === 0">Licensed Tour Guide – Ministry of Tourism</li>
            </ul>
          </div>
        </div>
      </div>
      ...
      <div class="mt-10 border-t border-border pt-8">
        ...
        <ul class="list-disc list-inside text-foreground space-y-2">
          <li v-for="(tour, index) in leader.highlight_tours" :key="index">{{ tour }}</li>
          <li v-if="!leader.highlight_tours || leader.highlight_tours.length === 0">Customized luxury tours available upon request.</li>
        </ul>
      </div>
      ...
      <div class="mt-10 border-t border-border pt-8">
        ...
        <div v-if="leader.reviews && leader.reviews.length > 0" class="space-y-4">
          <p v-for="(review, index) in leader.reviews" :key="index" class="italic text-muted-foreground">"{{ review }}"</p>
        </div>
        <p v-else class="text-muted-foreground">No reviews yet. Be the first to share your experience!</p>
      </div>
    </div>
...
const fetchTourLeader = async () => {
  try {
    loading.value = true;
    const leaderSlug = route.params.slug;
